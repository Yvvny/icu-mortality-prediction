...
def build_synthetic_model_df():
    ...
    return pd.DataFrame(
        {
            "anchor_age": [45, 51, 62, 70, 38, 59, 66, 74, 41, 57],
...
def test_data_processor_synthetic_model_columns():
    ...
    assert list(df.columns) == ["anchor_age", "gender_encoded", "los", "mortality"]
    assert len(df) == 4
...
def test_mortality_predictor_split_data_with_synthetic_input():
    ...
    assert len(predictor.X_train) == 7
    assert len(predictor.X_test) == 3
...
def test_train_logistic_regression_requires_split_first():
    ...
    with pytest.raises(Exception, match="Run split_data\\(\\) first"):
